...
Active: streaming/fresh graph indexes (FreshDiskANN line, Microsoft Research; StreamingDiskANN), out-of-distribution and drift-robust ANN (the big-ann-benchmarks "streaming" and "OOD" tracks, NeurIPS 2023+) *(frontier — verify)*. Work on **filtered + streaming** combined (label-aware updates) and on disk/SSD-resident dynamic graphs with bounded write amplification *(frontier — verify)*. Interest in formal analysis of $\alpha$-RNG repair and in incremental reconstruction to avoid full rebuilds is emerging at SIGMOD/VLDB *(frontier — verify)*. A pre-registered test of *when* to repair returned a **negative result**: triggering local edge repair on a measured navigability-degradation signal, rather than on a fixed clock, does **not** spend a fixed repair budget better. Against a fixed-cadence baseline run at the triggered policy's *realized* consolidation count, the tail-recall advantage is indistinguishable from zero at every operating point, graph degree and index scale on SIFT-128 and Fashion-MNIST-784 under bursty churn; at several points the clock is better. The earlier positive claim (≈2 pts SIFT, ≈5 pts Fashion-MNIST) traced to an **interpolated baseline**: recall is sharply concave in repair budget — one consolidation captures over half of all achievable gain — so reading the baseline off a straight line between zero and four consolidations manufactures a gap that matched-budget measurement removes. The methodological warning is the transferable part (Samyama, arXiv:2607.00728).
...
- **[SOTA]** Samyama Research. *When to Repair a Graph ANN Index: A Matched-Budget Negative Result, and the Interpolated-Baseline Trap That Hid It.* arXiv:2607.00728 (cs.IR), 2026. — [arXiv](https://arxiv.org/abs/2607.00728) · [code](https://github.com/samyama-ai/updatable-graph-index)
