...
A Bloom filter with $m$ bits over $k$ keys has false-positive rate $p \approx (1-e^{-(\ln 2) m/k})^{(\ln 2) m/k}$, minimized at $\approx 0.6185^{m/k}$, i.e. $\log_2(1/p) \approx 0.69\,(m/k)$ — needing $\ge \log_2 e \approx 1.44$× the information-theoretic floor of $k\log_2(1/p)$ bits. The allocation problem is the *Monkey* convex program: minimize total expected cost
