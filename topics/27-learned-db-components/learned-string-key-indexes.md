---
id: 27-learned-db-components/learned-string-key-indexes
title: "Learned String / Variable-Length Key Indexes"
...
**Why the numeric-CDF model breaks on strings.** Take $n=5$ keys: `apple, apply, apricot, banana, bandana`. Map the first $\ell=2$ bytes to a radix integer ($\texttt{ap}\to 24944$, $\texttt{ba}\to 25185$). Four of five keys collapse to the same radix value 24944 — the CDF is a *staircase*: a monotone linear model predicts essentially the same position for `apple`, `apply`, `apricot`, giving last-mile error $\varepsilon=\Theta(n)$ within the bucket. Extending to $\ell=7$ bytes separates them but multiplies the model's input domain and dilutes resolution.
